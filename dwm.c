...
#include <unistd.h>
#include <sys/types.h>
#include <sys/stat.h> /* autostart patch */
#include <sys/wait.h>
...
static void managealtbar(Window win, XWindowAttributes *wa);  /* Polybar patch */
static void managetray(Window win, XWindowAttributes *wa);    /* Polybar patch */
...
static void scantray(void); /* Polybar patch */
...
static void spawnbar(); /* Polybar patch */
...
static void unmanagealtbar(Window w); /* Polybar patch */
static void unmanagetray(Window w);   /* Polybar patch */
...
static int wmclasscontains(Window win, const char *class, const char *name); /* Polybar patch */
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				for (c = m->clients; c; c = c->next)
 				...
 				XMoveResizeWindow(dpy, m->barwin, m->wx, m->by, m->ww, m->bh); /* Polybar patch */
			}
			focus(NULL);
			arrange(NULL);
		}
...
setup(void)
{
	...
	lrpad = drw->fonts->h;
	bh = usealtbar ? 0 : drw->fonts->h + 2; /* Polybar patch */
	updategeom();
	/* init atoms */
...
updatebarpos(Monitor *m)
{
	...
	m->wh = m->mh;
	if (m->showbar) {
		m->wh -= m->bh; /* Polybar patch */
		m->by = m->topbar ? m->wy : m->wy + m->wh;
		m->wy = m->topbar ? m->wy + m->bh : m->wy; /* Polybar patch */
	} else
		m->by = -m->bh; /* Polybar patch */
}
...
wintomon(Window w)
{
	...
	for (m = mons; m; m = m->next)
		if (w == m->barwin || w == m->traywin) /* Polybar patch */
			return m;
	if ((c = wintoclient(w)))
		return c->mon;
	return selmon;
